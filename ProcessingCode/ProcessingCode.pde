...
void setup () {
  ...
  rolling = new SoundFile(this, "Loop_attempt.mp3");
...
void draw () {
  ...
  // detect if the ball is moving fast enough
  ...
  // if audio should be playing and was not start loop
  if (ballmoving && !wasPlaying) {
    rolling.loop();
    wasPlaying = true;
  // if audio should not be playing and was playing pause the audio
  } else if (!ballmoving && wasPlaying) {
    rolling.pause();
    wasPlaying = false;
  }

  // make audio play faster based off how fast its going
  if (ballmoving) {
